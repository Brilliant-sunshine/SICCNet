...
parser.add_argument('--subject', type=int, default=[1,2,3,4,5,6,7,8,9], help='3a=[1 2 3]')
...
parser.add_argument('--model_folder_CoSE', type=str, default="")
parser.add_argument('--head_path', type=str, default="E:\lzg1\image\\tsne\\2a\\head")
...
for backbone_net in args.backbone_net_all:
    for subject_id in args.subject:
        ...
        for index, fold in enumerate(args.folds[subject_id - 1]):
            ...
            path_shallow = "E:\lzg1\model_pth\\" + args.model_folder_shallow + "\\" + args.dataset + "\S" + str(subject_id) + \
                           "\S" + str(subject_id) + "F" + str(fold) + "epoch_" + str(
                            args.max_epochs - args.Is[subject_id - 1][0]) + ".pth"
            path_CoSE = "E:\lzg1\model_pth\\" + args.model_folder_CoSE + "\\" + args.dataset + "\S" + str(subject_id) + \
                           "\S" + str(subject_id) + "F" + str(fold) + "epoch_" + str(
                args.max_epochs - args.Is[subject_id - 1][0]) + ".pth"
            print(
            ...
            if backbone_net == 'shallow':
                ...
                model_CoSE = ShallowFBCSPNet(n_chans, n_classes, input_time_length=input_time_length,
                                            final_conv_length='auto')
            if cuda:
                model_shallow.cuda()
                model_CoSE.cuda()
            ...
            model_CoSE.load_state_dict(torch.load(path_CoSE))
            ...
            pred_fn1 = lambda x: var_to_np(model_CoSE(np_to_var(x).cuda()))
    ...
    plot_head_show(test_set, map_avg, map2, 10, 10, 10)